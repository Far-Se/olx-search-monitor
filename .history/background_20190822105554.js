				...
				.done(data => {
					...
					results.each(function() {
						...
						if (skip == 0) {
							...
							if ((searchData.indexOf('=907') > -1 || searchData.indexOf('=911') > -1) && searchData.indexOf('&mp') < 0) {
                                title = title.replace(/(?!.partament)[A-Z]\w{2,}( ([a-z0-9]+| ) [A-Z]\w+)?/g, (e) => {
                                    let forbidden = ['apartament','casa','proprietar','mutare','promo','camere','camera','euro','imediata','valabil','renovat','mobilat','utilat','apart','decomandat'];
                                    if(~forbidden.indexOf(e.toLowerCase()))
                                        return e;
                                     return `<span>${e}</span>`;
                                });
/* 								let regex = /(?!(.partament|.roprietar|.amar|.amer|Promo))[A-Z]\w{2,}( ([a-z0-9]+| ) [A-Z]\w+)?/gm;
								let m;
								...
								if ((m = regex.exec(title)) !== null) {
									...
									m.forEach((match, groupIndex) => {
                                        ...
                                        {
                                            console.log("x" + match);
                                            title = title.replace(match, `<u>${match}</u>`);
                                        }
									});
								} */
