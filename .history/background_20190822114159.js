						...
						if (skip == 0) {
							...
							if ((searchData.indexOf('=907') > -1 || searchData.indexOf('=911') > -1) && searchData.indexOf('&mp') < 0) {
								title = title.replace(/(?!(.partament|Casa))[A-Z]\w{2,}( ([a-z0-9]+| ) [A-Z]\w+)?/g, (e) => {
									let forbidden = ['apartament', 'casa', 'proprietar', 'mutare', 'promo', 'camere', 'camera', 'euro', 'imediata', 'valabil', 'renovat', 'mobilat', 'utilat', 'apart', 'decomandat', 'lux', 'ratp', 'central', 'ultracentral', 'valabil', 'vila', 'dou'];
