...
chrome.runtime.sendMessage({
	...
	for (let type in results) {
		...
		let td          = $('<td />');
		let div         = $('<div />');
        if(isset(results[type][0]['table'])) div.append('<table>');
		for (let result of results[type]) {
            if(isset(results[type][0]['table']))
            {
                div.append(`<tr><td>${result.table[0]}</td><td>${result.table[1]}</td><td>`);
            }
            ...
            `);
            if(isset(results[type][0]['table'])) div.append('</td></tr>');
