...
chrome.runtime.sendMessage({
	...
	for (let type in results) {
		...
		for (let result of results[type]) {
            ...
            {
                div.append(`<tr><td>${result.table[0]} mp</td><td>${result.table[1]} €</td>`);
            }
            
			div.append((results[type][0]['table'] ? '<td>': '')`
            ...
            `. (results[type][0]['table'] ? '</td>': ''));
            if(results[type][0]['table']) div.append('</td></tr>');
