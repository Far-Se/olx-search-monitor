...
class Background {
	...
	parseResults(cb = function() {}) {
		...
		for (let type in this.searchData) {
			...
			let data = this.searchData[type];
			let exclude = "";
			let include = "";
			if (data.indexOf('exclude') > -1) {
				exclude = data.match(/\&exclude=(.*?)($|&)/gmis)[0].replace(/(&|exclude=)/g, '');
			}
			if (data.indexOf('include') > -1) {
				include = data.match(/\&include=(.*?)($|&)/gmis)[0].replace(/(&|include=)/g, '');
			}
			toLoad++;

			$.post('https://www.olx.ro/ajax/bucuresti/search/list/', data)
				.done(data => {
					...
					results.each(function() {
						...
						if (skip == 0) {
                            ...
                            console.log(data);
							if (data.indexOf('&mp') > -1) {
                                console.log("in mp");
								let match = title.match(/\d+.mp/g);
								if (match) {
                                    console.log(match);
									title = `<span>${match[0]}<span> ${title}`;
									...
									$.get(href)
										.done(xdata => {
                                            let match = xdata.match(/\d+.m\W/gm);
                                            console.log(match);
											if (match) {
												title = `<span>${match[0]}<span> ${title}`;
												that.results[type].push({
