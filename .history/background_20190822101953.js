					...
					results.each(function() {
						...
						if (skip == 0) {
							...
							if ((searchData.indexOf('=907') > -1 || searchData.indexOf('=911') > -1) && searchData.indexOf('&mp') < 0) {
                                const regex = /(?!(.partament|.roprietar|.amar|.amer|Promo))[A-Z]\w{2,}( ([a-z0-9]+| ) [A-Z]\w+)?/gm;
let str = `73 000 €• Apartament 2 camere de vanzare Centru - Sfantul Lazar 1520 €`;
let m;
...
while ((m = regex.exec(str)) !== null) {
    ...
    m.forEach((match, groupIndex) => {
		if(match)
		str = str.replace(match,`<u>${match}</u>`);
    });
}
console.log(str);
